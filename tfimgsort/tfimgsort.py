...
import logging
import numpy as np
from .util import stringify, setup_dirs, write_csv_line, mv, ls
from .tfutil import classify, create_graph
...
confidence_dirs = ['high confidence',
                   'confident', 'low confidence', 'negative']
...
def sort_singleclass(img,
                     predictions,
                     target,
                     labels,
                     confidence_thresh,
                     directory):
    """Sort an image based on the confidence that the image matches target
    classification.
...
def run(unsorted_imgs,
    ...
    for img in imgs:
        ...
        try:
            ...
            mv(img, error_path)
            logger.error("There was a problem classifying %s.  It has been \
                        moved to the directory %s" % (img, error_path))
            continue
        ...
        if singleclass:
            sort_singleclass(img, predictions, singleclass,
                             labels, confidence_thresh, output_dir)
        elif multiclass:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Classifies images using a tensorflow based classifier')

    parser.add_argument("unsorted", type=str)
    parser.add_argument("--model-dir",
                        help="""The directory that contains the model you want
                             to use.  Defaults to `model`""",
                        type=str)
    parser.add_argument("--confidence-thresholds",
                        help="""A comma seperated list of thresholds for use
                             with singleclass classifications.  Defaults to
                             '0.9,0.7,0.5'""",
                        type=str)
    parser.add_argument("--csv",
                        help="""When classifying the images writes the results
                             to a given csv file""",
                        type=str)
    parser.add_argument("--singleclass",
                        help="""This is the default sorting scheme. Performs
                             binary classification on the images as the
                             specified class. The results are tiered into
                             folders based on confidence of result. You must
                             choose multiclass or singleclass but not both.""",
                        type=str)
    parser.add_argument("--multiclass",
                        help="""Performs multiclass classification, sorting the
                             input directory into the most likely class. You
                             must choose multiclass or singleclass but not
                             both.""",
                        action='store_true')
    parser.add_argument("--output-dir",
                        help="""The directory to output single or multiclass
                            sorting. Defaults to 'classifications'""",
                        type=str)
    ...
    if args.singleclass and args.multiclass:
        logger.critical(
            "Please use singleclass or multiclass, but not both.  See --help \
    ...
    if args.confidence_thresholds:
        confidence_thresh = list(
            map(float, args.confidence_thresholds.split(',')))
    else:
